...
class eNetModelBased(agent.FiniteHorizonAgent):
    ...
    def update_policy(self, k):
        ...
        action_axes = (self.state_action_dim[0]+1, self.state_action_dim[0]+2)
        for h in np.arange(self.epLen-1,-1,-1):
            if h == self.epLen - 1:
                self.qVals[h] = np.minimum(np.minimum(
                    self.qVals[h], self.epLen),
                    self.rEst[h] + self.scaling / np.sqrt(
                        self.num_visits[h]))
            else:
                vEst = np.sum(self.vVals[h + 1] * ((self.pEst[h] + self.alpha) / (
                        np.sum(self.pEst[h]) + len(self.state_net) * self.alpha)))
                self.qVals[h] = np.minimum(np.minimum(
                    self.qVals[h], self.epLen),
                    self.rEst[h] + self.scaling / np.sqrt(
                        self.num_visits[h]) + vEst)
        self.qVals = np.where(self.num_visits == 0, self.epLen, self.qVals)
        self.vVals = np.minimum(np.max(self.qVals,axis=action_axes),self.epLen)
